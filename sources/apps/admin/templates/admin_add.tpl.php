...
$(function(){
	...
	$("#username").formValidator({onshow:"<?php echo L('input').L('username')?>",onfocus:"<?php echo L('username').L('between_6_to_20')?>"}).inputValidator({min:6,max:20,onerror:"<?php echo L('username').L('between_6_to_20')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data :"app=admin&controller=admin&action=ajax_username",
		datatype : "html",
		async:'false',
...
<div class="content-menu ib-a blue line-x"><a href="?app=admin&controller=admin&action=init"><em><?php echo L('listadmins')?></em></a><span>|</span> <a href="?m=admin&c=administrator&a=add"  class="on"><em><?php echo L('add_admin')?></em></a></div>
</div>
<div class="pad-lr-10">
<form action="?app=admin&controller=admin&action=add" method="post" id="myform">
...
<td class="y-bg"><input type="text" class="input-text" name="email" value="" id="email" /></td>
</tr>
<tr>
<th><?php echo L('realname')?>：</th>
<td class="y-bg"><input type="text" class="input-text" name="realname" value="" /></td>
</tr>
<tr>
<th><?php echo L('mobile')?>：</th>
<td class="y-bg"><input type="text" class="input-text" name="mobile" value="" id="mobile"/></td>
</tr>
  <tr>
