...
class Web_Router {
	...
	public function match() {
		...
		$_pathinfo = trim ( str_replace ( Base_Request::get_base_url (), '', $full_url ), '/' );
		if (! $_pathinfo || ! preg_match_all ( '/^http[s]?:\/\/[^\/]+(\/\w+)?(\/\w+)?(\/\w+)?.*$/i', trim ( $_pathinfo, '/' ), $matches ) || strpos ( $_pathinfo, '.php' ) !== false || strpos ( $_pathinfo, Base_Request::get_host_info () . '/?' ) !== false) return null;
	...
	protected function set_params() {
		$this->action = Base_Request::get_request ( 'action' ) ? Base_Request::get_request ( 'action' ) : $this->_config ['action'];
		$this->controller = Base_Request::get_request ( 'controller' ) ? Base_Request::get_request ( 'controller' ) : $this->_config ['controller'];
		$this->application = Base_Request::get_request ( 'app' ) ? Base_Request::get_request ( 'app' ) : $this->_config ['application'];
	}
